<section class="bg-schoolsec">
  <div class="container">
    <div class="row pt-md-5">
      <div class="col-md-12 text-center pb-md-5">
        <h4 class="top-head">Hear from Our Students</h4>
      </div>
      <div class="col-md-12">
        <div id="owl-demo-mob" class="owl-carousel owl-theme">
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002582/website/testimonials/Selvakumar1_ee7za7.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">"Now I have a good exposure on building a website and knowledge over full
                    stack technology. During my training period I developed various projects, like tic-tac toe, quote
                    generator, To-do checklist, e-commerce. I got placed in a company through the interview provided by
                    Skill Safari."</p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.7/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Selvakumar</p>
                  <p class="studentdest pt-1">Phicode, Coimbatore</p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002582/website/testimonials/Krishnakumar1_ansdoa.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">"I was amazed with the kinds of technology that is out there in the market
                    but don't know how to use any of them . I was in a tight schedule trying hard to get enough time to
                    balance my work and studies. The trainers who are very well experienced and cooperative along with
                    supporting staff, made a lot of things easier in the journey."</p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.5/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Krishnakumar P B</p>
                  <p class="studentdest pt-1"></p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002582/website/testimonials/Aniket1_mlb3xx.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">"Skill Safari is an awesome skill development platform. I have learned
                    lots
                    of things, not only technical skills, but also real life things. All teams are very helpful in any
                    situation. Thank you, dear Skill Safari team for developing my career life" </p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.7/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Aniket Prakashrao Dhage</p>
                  <p class="studentdest pt-1"></p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002582/website/testimonials/Dharshini_xfjule.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">"The teaching from the beginning was carried out in the most professional
                    and friendly way. The trainers were very involved in helping me out with doubts at every time. I
                    feel great satisfaction in learning under you guys and glad that every penny that I spent is worth
                    the knowledge that you guys provided me. Its completely rare to have friendly trainers, its an
                    important factor that you guys excelled in." </p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.5/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Dharshini</p>
                  <p class="studentdest pt-1"></p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002581/website/testimonials/Meenaz_Farheen_s3nxim.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">"The best live educational platform I have ever come across. Each and
                    every mentor over here takes utmost care of each and every person and help in every way possible. I
                    really feel overwhelmed and so grateful being one of the students of Skill Safari. Thank you
                    Skill Safari!" </p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.5/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Meenaz Farheen Shaik</p>
                  <p class="studentdest pt-1"></p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002581/website/testimonials/Niharika_avgxr8.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">
                    "I got to know about skill safari from my college mentor and I'm very thankful for that. I wasn't
                    computer student in my schooling and puc, and I lacked self confidence, but from this program it
                    gave me self-confidence and most importantly I gained a lot of interest forever thankful for it."
                  </p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.5/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Niharika Kiran</p>
                  <p class="studentdest pt-1">TCS, Kochi</p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002581/website/testimonials/Krithi_Hemanth_jyher3.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">
                    "One thing that impressed me about Skill Safari is if I have a question there is always a quick
                    response. I’m already applying what I learned. Course content are Excellent, assignments that we
                    build increase our knowledge, mentoring approach were good. The instructors did a good job of
                    communicating and making it a more intimate arrangement."
                  </p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.5/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Krithi Hemanth</p>
                  <p class="studentdest pt-1"></p>
                </div>
              </div>
            </div>
          </div>
          <div class="item bg-white mt-3 pb-5 ps-md-2 pe-md-2">
            <div class="bbt mb-5">
              <div class="row">
                <div class="col-xl-12">
                  <img
                    src="https://res.cloudinary.com/skill-safari/image/upload/v1650002582/website/testimonials/Sumanth_oujh8q.jpg"
                    class="img-fluid w-100" alt="" title="">
                </div>
                <div class="col-xl-12 ps-4 pe-4">
                  <p class="school-para pt-4">
                    "We had a good conversation with mentors and the teaching skills were awesome and they were allowed
                    us to contact them for any doubt and at any time and this might won't happen many times as their
                    teaching was very good and we understood each and ever bit. It will be much worth each rupee you
                    paid and it also not cost too much."
                  </p>
                  <div class="starrating pt-1">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star-half"></i> </span>
                    <span class="rating">4.5/5</span>
                  </div>
                  <p class="studentname pt-3 mb-0">Sumanth Valiveti</p>
                  <p class="studentdest pt-1"></p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<!-- students section ended -->